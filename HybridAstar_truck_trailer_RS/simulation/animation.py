...
class Path:
    def __init__(self, x, y, yaw, yaw_trailer, direction):
        self.x = x # x position [m]
        self.y = y # y position [m]
        self.yaw = yaw # yaw angle [rad]
        self.yaw_trailer = yaw_trailer # trailer angle [rad]
...
def show_animation(path, oox, ooy, sx, sy, syaw0, syaw1, gx, gy, gyaw0, gyaw1):
    ...
    for ii in range(len(x)):
        ...
        if ii < len(x)-1:
            ...
            steer = 0.0
        plot_trailer(x[ii], y[ii], yaw[ii], yaw_trailer[ii], steer)
        plt.grid()
        plt.axis('equal')
        plt.pause(0.001)
    plt.show()

def plot_trailer(x, y, yaw, yaw_trailer, steer):
    ...
    Rot3 = np.asarray([[math.cos(yaw_trailer), math.sin(yaw_trailer)],[-math.sin(yaw_trailer), math.cos(yaw_trailer)]])
...
def main():
    x, y, yaw, yaw_trailer, direction = np.loadtxt('path.dat', delimiter=' ', unpack=True)
    path = Path(x, y, yaw, yaw_trailer, direction)
